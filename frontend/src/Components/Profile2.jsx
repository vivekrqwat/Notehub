...
const Profile = () => {
  ...
  const getleauge = (n) => {
    if (n >= 500 && n <= 800) {
      ...
      settitle({ league: "Professional Tier", title: "Subject Sensei" });
    } else {
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="bg-[#2a2a2a] rounded-xl shadow-md p-4">
            <div className="flex flex-wrap gap-4 mb-4">
              <button className="px-4 py-2 bg-[#1e1e1e] text-white rounded-md" onClick={() => setlistname("notes")}>
                Directories
              </button>
              <button className="px-4 py-2 bg-[#1e1e1e] text-white rounded-md" onClick={() => setlistname("posts")}>
                POSTs
              </button>
            </div>
            <div className="space-y-3 max-h-[500px] overflow-y-auto">
              {listdata?.map((item, idx) => (
                <div key={idx} className="flex-col bg-[#1e1e1e] ">
